...
### 4.13 Authentication (Optional)

- As a property owner, I want the app to open directly without requiring a login when I'm the only user on a private machine, so I can access my data faster.
- As a property owner, I want to enable login protection from Settings when needed, so I can secure my financial data if others use the same computer.
- As a property owner, I want the app to remember my credentials so I don't have to re-type them every time I open the app.
- As a property owner, I want to disable the login requirement from Settings if I no longer need it, so the app opens directly again.
...
| FR-SET-11  | **Security toggle:** Enable or disable the login requirement on app startup via Settings → Security. When disabled (default), the app launches directly to the Dashboard without authentication. When enabled, the user must create an admin account (first launch) or log in with existing credentials on every startup. Changes take effect on the next restart |
...
| **Security**           | NFR-SEC-02    | **Optional authentication:** A `require_auth` setting (default OFF) in Settings → Security controls whether the app requires login on startup. When OFF, the app launches directly to the Dashboard with no login screen. When ON, the user must authenticate before any data access. Credentials are stored as a single admin account (bcrypt-hashed passwords in the `users` table). An optional "Remember me" checkbox on the login page encrypts credentials via Electron `safeStorage` (OS keychain: Windows DPAPI / macOS Keychain / Linux Keyring) and stores them in a local file for auto-fill on next launch. The setting change takes effect on the next app restart. |
...
├── Notification Center
├── Settings
│   ├── Appearance & Language (language, theme, font size)
│   ├── Security (require login toggle)
│   ├── Financial Defaults (reporting currency, payment method)
│   ├── Reminders (due date, contract end, document expiry, recurring expense)
│   ├── Backup & Data (backup path)
...
| require_auth                           | INTEGER DEFAULT 0 | `0` = auth disabled (default, app opens directly); `1` = auth required on every launch        |
...
| ------------- | ------------- | ------------------------------------------------------------- |
| id            | INTEGER PK    | Auto-increment primary key                                    |
| username      | TEXT UNIQUE   | Login username (3–50 chars, alphanumeric + underscore/hyphen) |
| password_hash | TEXT NOT NULL | bcrypt hash of the password (10 rounds)                       |
| display_name  | TEXT          | Optional display name                                         |
| is_active     | INTEGER       | `1` = active (default); `0` = disabled account                |
...
- **Components:** Sections: Appearance & Language, Security, Financial Defaults, Reminders, Backup & Data.
  - **Appearance & Language section:** Language selector (Arabic / English radio buttons), theme (Light / Dark), font size (Small / Medium / Large). Selecting a language immediately applies RTL or LTR layout to the entire app.
  - **Security section:** A "Require login on startup" toggle (Switch). When enabled, an info banner explains that the change takes effect on the next restart. If the user enables auth but has not yet created an admin account, the next launch will show the first-time registration form.
- **Validation:** `app_language` must be `ar` or `en`; `require_auth` must be `0` or `1`.

### 9.10b Login / Registration Screen

- **Purpose:** Authenticate the user when `require_auth` is enabled in Settings. Shown only when auth is required and no active session exists.
- **Components:**
  - **Login mode** (users exist): Username field, Password field (with show/hide toggle), "Remember me" checkbox, Login button.
  - **Registration mode** (no users exist — first launch): Username field, Display Name field (optional), Password field, Confirm Password field, Create Account button.
  - The mode is determined automatically by checking whether any user account exists in the database.
- **Actions:** Login (verifies credentials via bcrypt), Register (creates the single admin account), "Remember me" toggle.
- **Validation:** Username: required, 3–50 chars, alphanumeric + underscore/hyphen. Password: required, 6–128 chars. Confirm password must match. Only one admin account is permitted (single-user architecture).
- **Remember Me:** When checked, credentials are encrypted via Electron `safeStorage` (OS keychain) and persisted in `userData/saved-credentials.json`. On next launch, fields are pre-filled and the checkbox is pre-checked. When unchecked, saved credentials are cleared.
- **Error States:** "Invalid username or password" (i18n key), "This account has been disabled" (i18n key), "Registration is disabled — an admin account already exists" (i18n key).
- **Navigation:** Shown by the `AuthGate` component before the router mounts; upon successful login, the full app renders.
...
### 11.6b Workflow: Authentication — Optional Login on Startup

```mermaid
flowchart TD
    A[App launches] --> B{require_auth setting?}
    B -- OFF / default --> C[AuthGate auto-sets system user in context]
    C --> D[Dashboard renders directly — no login screen]
    B -- ON --> E{Users exist in DB?}
    E -- No users yet --> F[Registration form shown: username + display name + password]
    F --> G[User creates admin account]
    G --> H[User logged in — Dashboard renders]
    E -- Users exist --> I[Login form shown: username + password]
    I --> J{Remember me checked?}
    J -- Yes --> K[Saved credentials auto-fill fields]
    J -- No --> L[Empty fields — user types credentials]
    K --> M[User clicks Login]
    L --> M
    M --> N{Credentials valid?}
    N -- Yes --> O{Remember me?}
    O -- Yes --> P[Credentials encrypted via safeStorage and saved to disk]
    O -- No --> Q[Saved credentials cleared from disk]
    P --> H
    Q --> H
    N -- No --> R[Error message displayed — user retries]
```
...
_End of document — Version 1.1 — Ready for direct use as a reference for a team or AI development tool building the application._
